...
describe Pbl::Models::Comment do
  ...
  describe '.create' do
    context 'successful' do
      let(:params) { {} }
      before(:each) do
        stub_request(:post, 'http://0.0.0.0:3001/comments').to_return(
  ...
  describe '.update' do
    context 'successful' do
      ...
      before(:each) do
        stub_request(:patch, 'http://0.0.0.0:3001/comments/1').to_return(
            body: nil,
            status: 200
        )
      end
  ...
  describe '.destroy' do
    before(:each) do
      stub_request(:delete, 'http://0.0.0.0:3001/comments/1').to_return(
          body: nil,
          status: 200
      )
    end
  ...
  describe '.find!' do
    before(:each) do
      stub_request(:get, 'http://0.0.0.0:3001/comments/1').to_return(
          body: clazz.new(default_params[:object]).to_json,
          status: 200
      )
      stub_request(:get, 'http://0.0.0.0:3001/comments/2').to_return(
          body: nil,
          status: 404
      )
    end
    ...
    context 'project exist' do
      ...
      it 'find' do
        ...
        expect(clazz_instance.title).to eq('title')
      end
    end
  ...
  describe '.where' do
    before(:each) do
      ...
      stub_request(:get, 'http://0.0.0.0:3001/comments/').to_return(
