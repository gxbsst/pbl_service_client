...
describe Pbl::Models::Assignments::Score do

  shared_examples 'collect clazz_instance' do
    it { expect(clazz_instance.comment).to eq('comment') }
    it { expect(clazz_instance.score).to eq('score') }
  end
  ...
  describe '.create' do
    context 'successful' do
      let(:params) { {} }
      before(:each) do
        stub_request(:post, 'http://0.0.0.0:3001/assignment/scores').to_return(
  ...
  describe '.update' do
    context 'successful' do
      ...
      before(:each) do
        stub_request(:patch, 'http://0.0.0.0:3001/assignment/scores/1').to_return(
            body: nil,
            status: 200
        )
      end
  ...
  describe '.destroy' do
    before(:each) do
      stub_request(:delete, 'http://0.0.0.0:3001/assignment/scores/1').to_return(
          body: nil,
          status: 200
      )
    end
  ...
  describe '.find!' do
    before(:each) do
      stub_request(:get, 'http://0.0.0.0:3001/assignment/scores/1').to_return(
          body: clazz.new(default_params[:object]).to_json,
          status: 200
      )
      stub_request(:get, 'http://0.0.0.0:3001/assignment/scores/2').to_return(
          body: nil,
          status: 404
      )
    end
    ...
    context 'project exist' do
      ...
      it 'find' do
        ...
        expect(clazz_instance.comment).to eq('comment')
        expect(clazz_instance.score).to eq('score')
      end
    end
  ...
  describe '.where' do
    before(:each) do
      ...
      stub_request(:get, 'http://0.0.0.0:3001/assignment/scores/').to_return(
    ...
    it { expect(clazz_instances.fetch(:data).first.comment).to eq('comment') }
