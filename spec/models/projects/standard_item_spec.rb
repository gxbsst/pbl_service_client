...
describe Pbl::Models::Projects::StandardItem  do

  shared_examples 'collect clazz_instance' do
    it { expect(clazz_instance.project_id).to eq('project_id') }
    it { expect(clazz_instance.standard_item_id).to eq('standard_item_id') }
  end
  ...
  describe '.create' do
    context 'successful' do
      let(:params) { {} }
      before(:each) do
        stub_request(:post, 'http://0.0.0.0:3001/pbl/standard_items').to_return(
  ...
  describe '.update' do
    context 'successful' do
      ...
      before(:each) do
        stub_request(:patch, 'http://0.0.0.0:3001/pbl/standard_items/1').to_return(
          body: nil,
          status: 200
        )
      end
  ...
  describe '.destroy' do
    before(:each) do
      stub_request(:delete, 'http://0.0.0.0:3001/pbl/standard_items/1').to_return(
        body: nil,
        status: 200
      )
    end
  ...
  describe '.find!' do
    before(:each) do
      stub_request(:get, 'http://0.0.0.0:3001/pbl/standard_items/1').to_return(
        body: clazz.new(default_params[:object]).to_json,
        status: 200
      )
      stub_request(:get, 'http://0.0.0.0:3001/pbl/standard_items/2').to_return(
        body: nil,
        status: 404
      )
    end
    ...
    context 'project exist' do
      ...
      it 'find' do
        ...
        expect(clazz_instance.project_id).to eq('project_id')
        expect(clazz_instance.standard_item_id).to eq('standard_item_id')
      end
    end
  ...
  describe '.where' do
    before(:each) do
      ...
      stub_request(:get, 'http://0.0.0.0:3001/pbl/standard_items/').to_return(
    ...
    it { expect(clazz_instances.fetch(:data).first.project_id).to eq('project_id') }
