...
describe Pbl::Models::Skills::Technique do

  shared_examples 'collect clazz_instance' do
    it { expect(clazz_instance.title).to eq('title') }
    it { expect(clazz_instance.sub_category_id).to eq('sub_category_id') }
  ...
  describe '.create' do
    context 'successful' do
      let(:params) { {} }
      before(:each) do
        stub_request(:post, 'http://0.0.0.0:3001/skill/techniques').to_return(
  ...
  describe '.update' do
    context 'successful' do
      ...
      before(:each) do
        stub_request(:patch, 'http://0.0.0.0:3001/skill/techniques/1').to_return(
          body: nil,
          status: 200
        )
      end
  ...
  describe '.destroy' do
    before(:each) do
      stub_request(:delete, 'http://0.0.0.0:3001/skill/techniques/1').to_return(
        body: nil,
        status: 200
      )
    end
  ...
  describe '.find!' do
    before(:each) do
      stub_request(:get, 'http://0.0.0.0:3001/skill/techniques/1').to_return(
        body: clazz.new(default_params[:object]).to_json,
        status: 200
      )
      stub_request(:get, 'http://0.0.0.0:3001/skill/techniques/2').to_return(
        body: nil,
        status: 404
      )
    end
