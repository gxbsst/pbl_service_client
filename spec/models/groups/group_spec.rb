...
describe Pbl::Models::Groups::Group do
  ...
  shared_examples 'collect clazz_instance' do
    ...
    it { expect(clazz_instance.description).to eq('description') }
  end
  ...
  describe '.create' do
    context 'successful' do
      let(:params) { {} }
      before(:each) do
        stub_request(:post, 'http://0.0.0.0:3001/group/groups').to_return(
  ...
  describe '.update' do
    context 'successful' do
      ...
      before(:each) do
        stub_request(:patch, 'http://0.0.0.0:3001/group/groups/1').to_return(
          body: nil,
          status: 200
        )
      end
    ...
    context 'failed' do
      let(:update_params) { {name: 'update level' }}
      let(:return_body) {
        body =  {error: {name: ['name error']}}
        JSON.generate(body)
      }
  ...
  describe '.destroy' do
    before(:each) do
      stub_request(:delete, 'http://0.0.0.0:3001/group/groups/1').to_return(
        body: nil,
        status: 200
      )
    end
  ...
  describe '.find!' do
    before(:each) do
      stub_request(:get, 'http://0.0.0.0:3001/group/groups/1').to_return(
        body: clazz.new(default_params[:object]).to_json,
        status: 200
      )
      stub_request(:get, 'http://0.0.0.0:3001/group/groups/2').to_return(
        body: nil,
        status: 404
      )
    end
    ...
    context 'project exist' do
      ...
      it 'find' do
        ...
        expect(clazz_instance.description).to eq('description')
  ...
  describe '.where' do
    before(:each) do
      ...
      stub_request(:get, 'http://0.0.0.0:3001/group/groups/').to_return(
